        ...
        <ul class="list-group">
            @foreach($exam->questions as $question)
                <li class="list-group-item">
                    ...
                    <form action="{{ route('teacher.questions.update', $question->id) }}" method="POST" class="mt-3 d-inline">
                        @csrf
                        @method('PUT')
                    ...
                    <ul class="mt-2">
                        @foreach($question->options as $option)
                            <li>
                                ...
                                <form action="{{ route('teacher.options.update', $option->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('PUT')
                                ...
                                <form action="{{ route('teacher.options.destroy', $option->id) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="btn btn-danger btn-sm">حذف</button>
                                </form>
                    ...
                    @if($question->options->count() < 4)
                        <!-- إضافة خيارات للسؤال -->
                        <form action="{{ route('teacher.options.store') }}" method="POST" class="mt-3">
                            @csrf
